...
class FileFeedStorageTest(unittest.TestCase):
    ...
    def test_overwrite(self):
        ...
        return self._assert_stores(
            FileFeedStorage(path, feed_options={"overwrite": True}), path
        )
    ...
    def test_export_feed_export_fields(self):
        ...
        for item_cls in [self.MyItem, dict]:
            ...
            yield self.assertExported(
                items, ["foo", "baz", "egg"], rows, settings=settings, ordered=True
            )
...
class BatchDeliveriesTest(FeedExportTestBase):
    ...
    def assertExportedJsonLines(self, items, rows, settings=None):
        ...
        settings.update(
            {
                "FEEDS": {
                    os.path.join(self._random_temp_filename(), "jl", self._file_mark): {
                        "format": "jl"
                    },
                },
            }
