...
try:
    ...
    TestDataClass = make_dataclass(
        "TestDataClass", [("name", str), ("url", str), ("price", int)]
    )
...
class EngineTest(unittest.TestCase):
    ...
    def test_crawler_change_close_reason_on_idle(self):
        ...
        yield self.run.run()
        self.assertEqual(
            {"spider": self.run.spider, "reason": "custom_reason"},
    ...
    def _assert_scheduled_requests(self, count=None):
        ...
        self.assertEqual(len(self.run.reqreached), responses_count)
    ...
    def _assert_signals_caught(self):
        ...
        self.assertEqual(
            {"spider": self.run.spider}, self.run.signals_caught[signals.spider_opened]
        )
        self.assertEqual(
            {"spider": self.run.spider}, self.run.signals_caught[signals.spider_idle]
        )
        self.assertEqual(
            {"spider": self.run.spider, "reason": "finished"},
            self.run.signals_caught[signals.spider_closed],
        )
